-- Миграция 001: Настройка pgvector и гибридного поиска
-- Выполнить на Supabase
...
-- 2) Добавляем колонку для векторов (если embedding хранился JSON-строкой)
ALTER TABLE public.kb_chunks
  ADD COLUMN IF NOT EXISTS embedding_vec vector(1536);

-- 3) Добавляем колонку для полнотекстового поиска
ALTER TABLE public.kb_chunks
  ADD COLUMN IF NOT EXISTS content_tsv tsvector;

-- 4) Создаем функцию для обновления tsvector
...
-- 5) Создаем триггер для поддержания tsvector
