# Исправленный скрипт для развертывания на VM
...
try {
    ...
    Invoke-VMCommand "docker-compose logs --tail=20 bot-backend"

    # 8. Тестируем API
    Write-Host "🧪 Тестируем RAG API..." -ForegroundColor Yellow
    try {
        $response = Invoke-RestMethod -Uri "http://$VM_HOST:3000/api/supabase-rag/health" -Method Get -TimeoutSec 10
