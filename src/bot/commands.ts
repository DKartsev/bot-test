...
bot.start((ctx) =>
  ctx.reply("Привет! Я на связи 🙌 Напишите /ticket, чтобы создать обращение."),
);
bot.help((ctx) => ctx.reply("Доступно: /start, /help, /ticket"));
bot.command("ticket", async (ctx) => {
  await ctx.reply(
    "Опишите проблему одним сообщением. Мы создадим тикет и свяжемся 🙌",
  );
});
bot.on("text", async (ctx) => {
  ...
  try {
    ...
    await ctx.reply(
      "Произошла ошибка. Попробуйте позже или используйте /ticket.",
    );
  }
});
