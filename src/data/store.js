...
const DEFAULT_BASE = path.join(
  ...
  'data',
  'tenants',
  process.env.TENANT_DEFAULT_ID || 'default'
);
...
function createStore(basePath = DEFAULT_BASE) {
  let resolvedBase = basePath || DEFAULT_BASE;
  const parent = path.dirname(resolvedBase);
  if (resolvedBase.endsWith(path.sep + 'root') && fs.existsSync(path.join(parent, 'qa_pairs.json'))) {
    resolvedBase = parent;
  }
  const candidate = path.join(resolvedBase, 'qa_pairs.json');
  if (!fs.existsSync(candidate) && fs.existsSync(path.join(parent, 'qa_pairs.json'))) {
    resolvedBase = parent;
  }
  if (stores.has(resolvedBase)) return stores.get(resolvedBase);

  const emitter = new EventEmitter();
  let qaPairs = [];

  function dataFile() {
    return path.join(resolvedBase, 'qa_pairs.json');
  }
  function tempFile() {
    return path.join(resolvedBase, 'qa_pairs.tmp.json');
  }
  function backupDir() {
    return path.join(resolvedBase, 'backups');
  }
  ...
  function load() {
    ...
    try {
      ...
      qaPairs = parsed.map((item) => {
        ...
        if (!item.id || !item.status || !item.createdAt || !item.updatedAt) changed = true;
