...
export default function ChatWindow({ selectedChatId, onSendMessage }: ChatWindowProps) {
	...
	const getMessageStyle = (role: ChatMessage['role']) => {
		switch (role) {
			case 'user':
				return 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white ml-auto max-w-xs';
			case 'assistant':
				return 'bg-blue-100 dark:bg-blue-900/30 text-blue-900 dark:text-blue-100 max-w-xs';
			case 'operator':
				return 'bg-green-100 dark:bg-green-900/30 text-green-900 dark:text-green-100 ml-auto max-w-xs';
			default:
	...
	const handleChangeStatus = async (next: Chat['status']) => {
		...
		try {
			await updateChatStatus(selectedChatId, next);
		} finally {
	...
	return (
		...
		>
				...
				<div className="flex items-center justify-between">
					...
					<div className="flex items-center gap-2">
						...
						<motion.button
							...
							whileTap={{ scale: 0.95 }}
							className="p-2 text-gray-600 dark:text-gray-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl transition-colors"
						>
							<Phone size={20} />
						</motion.button>
						<motion.button
						...
						<motion.button
							whileHover={{ scale: 1.05 }}
							whileTap={{ scale: 0.95 }}
							className="p-2 text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl transition-colors"
						>
			...
			<div className="flex-1 overflow-y-auto p-4 bg-gray-50 dark:bg-gray-900">
				{isFetching && (
					<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-6 text-gray-500 dark:text-gray-400">
				...
				{!isFetching && error && (
					<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-6 text-red-500 dark:text-red-400">
					...
					<AnimatePresence>
						{messages.map((msg, index) => (
							<motion.div
								...
								className={`mb-4 flex ${msg.role === 'operator' ? 'justify-end' : 'justify-start'}`}
							>
			...
			<div className="p-4 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
				<div className="flex items-end gap-3">
					<motion.button
						whileHover={{ scale: 1.05 }}
						whileTap={{ scale: 0.95 }}
						className="p-3 text-gray-600 dark:text-gray-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl transition-colors"
					>
						<Paperclip size={20} />
					</motion.button>
					...
					<div className="flex-1 relative">
						<textarea
							...
							rows={1}
							className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-2xl bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
