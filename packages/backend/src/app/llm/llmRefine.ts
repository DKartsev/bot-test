...
export async function refineDraft(
	draft: BotDraft,
	opts: { targetLang?: string; temperature?: number; minConfidenceToEscalate?: number } = {}
) {
	...
	const chat = await client.chat.completions.create({
		...
		messages: [
			...
			{ role: "user", content: "Материалы ниже. Верни СТРОГО JSON по указанной схеме, без пояснений." },
